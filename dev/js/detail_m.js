$(document).ready(function () {
  ...
  $.each(pd, function (i, d) {
    if (d.name == name) {
      $(".left-side .item-container .title .cls-1").css("fill", d.정당색);
    }
  });
  ...
  $.each(targetData, function (i, d) {
    ...
    if (d.movie) {
      leftHtml += `<div class="movie-container">
        <video height="300" width="100%"autoplay controls>
    ...
    leftHtml += `<div class="item-container">`;

    leftHtml += `<div class="speech">${d.year}</div>`;

    leftHtml += `<div class="title">
    ...
    leftHtml += `<div class="text ${d.type}"><img src="img/quote.png" class="quote-img">${d.text}</div></div>`;
    console.log(d.title);
  });
        ...
        point: {
          events: {
            mouseOver: function () {
              ...
              if (this.movie) {
                $(".event-video").html(`
                <video height="420" width="100%" autoplay muted>
